$(document).ready(function () {
    ...
    var table = $("#contrato").DataTable({
        ...
        columns: [{ data: "contrato" }, { data: "status" }, { data: "btn" }],
        responsive: {
            breakpoints: [
                {
                    name: "desktop",
    ...
    $(document).on("click", ".view", function (e) {
        ...
        var caperturaS = $("#cAperturaInput").val();
        caperturaS = caperturaS.slice(3);
        $("#cAperturaInput").val(caperturaS);
        // $("#cAperturaInput").prop("readonly", true);

        cmensual = cmensual.toString().replace(",", ".");
        $("#cMensualInput").val(cmensual);
        var cmensualS = $("#cMensualInput").val();
        cmensualS = cmensualS.slice(3);
        $("#cMensualInput").val(cmensualS);
    ...
    $(document).on("click", ".edit", function (e) {
        ...
        $("#PagoComisionSerie1Input").val(pagocomisionS1);
        $("#PagoComisionSerie1Input").prop("readonly", false);

        $("#PagoComisionSerieNInput").val(pagocomisionSN);
        $("#PagoComisionSerieNInput").prop("readonly", false);
        ...
        if (target.is("#inversionInput")) {
            ...
            if (acc == "edit") {
                if (
                    ...
                    dataInversionUS != $("#inversionInput").val()
                ) {
                    $("#contratoForm").attr("action", "/admin/addContrato");
                }
            }
            ...
            if (acc == "edit") {
                if (
                    $("#statusInput").val() == "Refrendado" &&
                    dataInversionMXN != $("#inversionUsInput").val()
                ) {
                    $("#contratoForm").attr("action", "/admin/addContrato");
                }
            }
        ...
        if (
            ...
            for (var i = 0; i < meses; i++) {
                ...
                if (parseInt(fecha[1]) + 1 == 2) {
                    if (fechaFeb[2] == 30 || fechaFeb[2] == 31) {
                        fecha = `28/02/${fecha[0]}`;
                    } else {
