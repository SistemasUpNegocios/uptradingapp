...
class ReportePagoClienteAnteriorController extends Controller
{
    ...
    public function index()
    {
        if(auth()->user()->is_root || auth()->user()->is_admin || auth()->user()->is_procesos || auth()->user()->is_egresos || auth()->user()->is_ps_diamond){        
            return View('reportepagoclienteanterior.show');
        }else{
    ...
    public function getResumenPagoClienteMensual(Request $request)
    {
        ...
        return response()->view('reportepagoclienteanterior.tabla', $data, 200);
    }
    ...
    public function imprimirResumenCliente(Request $request)
    {
        ...
        $pdf = PDF::loadView('reportepagoclienteanterior.imprimir', $data);
    ...
    public function getReportePago(Request $request)
    {
        ...
        $pdf = PDF::loadView('reportepagoclienteanterior.reporte', $data);
    ...
    public function tiposPagosClientes(Request $request)
    {
        ...
        $pdf = PDF::loadView('reportepagoclienteanterior.pagos', $data);
    ...
    public function editStatus(Request $request)
    {
        ...
        if($request->status == "Pagado"){
            ...
            $pago = new Pago;
            $pago->id_contrato = $request->contratoid;
            $pago->memo = "Pago a cliente ($request->pago)";
            $pago->monto = $request->rendimiento.",";
            $pago->tipo_pago = "efectivo,";
            $pago->tipo_cambio = number_format($request->dolar, 2);
            $pago->save();
