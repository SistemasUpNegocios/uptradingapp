...
class ContratoController extends Controller
{
    ...
    public function addContrato(Request $request)
    {
        if ($request->ajax()) {
            ...
            $contrato->tipo_cambio_eur = $request->input('tipo_cambio_eur');
            $contrato->tipo_cambio_chf = $request->input('tipo_cambio_chf');
            ...
            $contrato->inversion_eur = $request->input('inversion_eur');
            $contrato->inversion_letra_eur = $request->input('inversion_letra_eur');
            ...
            $contrato->firma_electronica = $request->input('firma_electronica');

            if($request->cambiar_porcentaje == "SI"){
                $contrato->cambiar_porcentaje = "SI";
                $contrato->porcentaje_rendimiento = $request->porcentaje_rendimiento;
            }else{
            ...
            }else if($request->moneda == "euros"){
                $tipo_cambio->valor = $request->tipo_cambio_eur;
            }else if($request->moneda == "francos"){
                $tipo_cambio->valor = $request->tipo_cambio_chf;
            }
            ...
            for ($i = 0; $i <= 4; $i++) {
                if ($request->input('nombre-ben' . $i)) {
                    ...
                    $beneficiario->parentesco = strtoupper($request->input($parentesco));
            ...
            if($contrato->moneda == "dolares"){
                ...
                $pago = $contrato->inversion_eur;
                $monto = $contrato->inversion_eur;
            }elseif($contrato->moneda == "francos"){
                $pago = $contrato->inversion_chf;
                $monto = $contrato->inversion_chf;
            }else{
                $pago = $contrato->inversion_us;
                $monto = $contrato->inversion_us;
            }
            ...
            if ($tipo_contrato[0]->tipo == "Rendimiento compuesto") {
                for ($i = 0; $i < $periodo; $i++) {
                    ...
                    if ($i == 0) {
                        ...
                        $pagops->fecha_pago = $fecha_pago;
                        $pagops->fecha_limite = $fecha_limite;
                        $pagops->pago = round(($monto * $capertura), 2);
                        $pagops->status = 'Pendiente';
                        $pagops->memo = 'Comisión por apertura';
                        $pagops->tipo_pago = 'Pendiente';
                        ...
                        $pagops->fecha_pago = $fecha_pago;
                        $pagops->fecha_limite = $fecha_limite;
                        $pagops->pago = round(($monto * $cmensual), 2);
                        $pagops->status = 'Pendiente';
                        $pagops->memo = 'Comisión mensual';
                        $pagops->tipo_pago = 'Pendiente';
    ...
    public function enviarTelegram(Request $request)
    {
        ...
        $aviso = $contrato[0];
        $link = "https://admin.uptradingexperts.com/admin/contrato?con=".$contrato[1];
        $mensaje = "$aviso\nContrato: <a href='$link'>$contrato[1]</a>";
