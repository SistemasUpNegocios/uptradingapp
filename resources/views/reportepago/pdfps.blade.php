...
$primerDia = $fecha->firstOfMonth();
$primerDia = $primerDia->toDateString();

$ultimoDia = $fecha->lastOfMonth();
$ultimoDia = $ultimoDia->toDateString();

$fecha = $fecha->toDateString();

$fecha_texto = \Carbon\Carbon::parse(strtotime($fecha))->formatLocalized('%B de %Y');

$totalPeriodo = DB::table("pago_ps")
->select(DB::raw("SUM(pago) AS totalPeriodo"))
->where("pago_ps.fecha_pago", ">=", $primerDia)
->where("pago_ps.fecha_pago", "<=", $ultimoDia) ->get();
    @endphp
    ...
    <div class="contenedor_imprimir_contrato2 w-100">
        <p class="mb-3 contrato_parrafo text-center mt-5" style="font-weight: bold; text-transform: uppercase;">Reporte
            mensual de pagos a PS {{ $fecha_texto }} ({{ $primerDia }} - {{ $ultimoDia }})</p>
        ...
        ->where("pago_ps.fecha_pago", "<=", $ultimoDia) ->get();
            ...
            ->where("pago_ps.fecha_pago", ">=", $primerDia)
            ->where("pago_ps.fecha_pago", "<=", $ultimoDia) ->where("pago_ps.tipo_pago", "=", 'Efectivo')
                ->get();

                $totSwiss = DB::table("pago_ps")
                ->select(DB::raw("SUM(pago) AS totalSwiss"))
                ->where("pago_ps.fecha_pago", ">=", $primerDia)
                ->where("pago_ps.fecha_pago", "<=", $ultimoDia) ->where("pago_ps.tipo_pago", "=", 'Transferencia
                    Swissquote a POOL')
                    ->get();
                    ...
                    ->where("pago_ps.fecha_pago", ">=", $primerDia)
                    ->where("pago_ps.fecha_pago", "<=", $ultimoDia) ->where("pago_ps.tipo_pago", "=", 'Transferencia MX
                        a POOL')
                        ->get();
                    ...
                    ->where("pago_ps.fecha_pago", ">=", $primerDia)
                    ->where("pago_ps.fecha_pago", "<=", $ultimoDia) ->where("pago_ps.tipo_pago", "=", 'Transferencia HSBC
                        a POOL')
                        ->get();
                        @endphp
