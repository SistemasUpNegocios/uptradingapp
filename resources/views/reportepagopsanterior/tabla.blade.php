<div class="mt-4 contDolar" style="overflow-x: auto;">
    <table class="resumenPagoPs table table-striped table-bordered nowrap text-center" style="width: 100%; vertical-align: middle" id="resumenPagoPs">
            ...
            @foreach ($lista_ps as $ps)
                @php
                    ...
                    $status_pago = DB::table('contrato')
                        ->join('ps', 'ps.id', '=', 'contrato.ps_id')
                        ->join('pago_ps', 'pago_ps.contrato_id', '=', 'contrato.id')
                        ->select('pago_ps.id', 'pago_ps.status', 'contrato.id AS contratoid', 'pago_ps.serie')
                        ->where('pago_ps.fecha_limite', 'like', "$fecha%")
                        ->where('contrato.ps_id', $ps->id)
                        ->where("contrato.status", "!=", "Activado")
                        ->where('ps.codigoPS', '!=', "IA1")
                    ...
                    $status_pago_convenio = DB::table('convenio')
                        ->join('ps', 'ps.id', '=', 'convenio.ps_id')
                        ->join('pago_ps_convenio', 'pago_ps_convenio.convenio_id', '=', 'convenio.id')
                        ->select('pago_ps_convenio.id', 'pago_ps_convenio.status', 'convenio.id AS convenioid', 'pago_ps_convenio.serie')
                        ->where('pago_ps_convenio.fecha_limite', 'like', "$fecha%")
                        ->where('convenio.ps_id', $ps->id)
                        ->where('convenio.status', '!=', 'Activado')
                        ->where('ps.codigoPS', '!=', "IA1")
                    ...
                    $pagos_convenio = "";
                    $status_convenio = "";
                    foreach ($status_pago_convenio as $status) {
                        $pagos_convenio .= $status->id.',';

                        if(strlen($status->status) > 0){
                            $status_convenio = $status->status;
                        }
                    } 
                    ...
                    $convenioid = "";
                    foreach ($status_pago_convenio as $convenio) {
                        $convenioid .= $convenio->convenioid.',';
                    }

                    $contratoid = $contratoid.$convenioid;

                    $numero_pago = "";
                    foreach ($status_pago as $pago) {
                        $numero_pago .= str_pad($pago->serie, 2, "0", STR_PAD_LEFT).'/12'.',';
                    }

                    $numero_pago_convenio = "";
                    foreach ($status_pago_convenio as $pago) {
                        $numero_pago_convenio .= str_pad($pago->serie, 2, "0", STR_PAD_LEFT).'/12'.',';
                    }
                ...
                @if ($comision_ps > 0)
                    <tr>
                        ...
                        <td class="d-flex align-items-center">
                            <button title="Información de pago" class="btn btn-dark nota ms-1" style="font-size: 13px; padding: 7px" data-pagoid="{{$pagos}}" data-contratoid="{{$contratoid}}" data-pesos="{{ $comision }}" data-dolares="{{$comision_dolares}}" data-numeropago="{{$numero_pago}}" data-numeropagoconvenio="{{$numero_pago_convenio}}" data-pagoidconvenio="{{$pagos_convenio}}" data-moneda="dolares"> <i class="bi bi-journal-text"></i></button>

                            <button class="btn btn-warning ms-1" style="font-size: 13px; padding: 7px" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-fecha="{{ $fecha }}" data-moneda="dolares" title="Imprimir pago" id="imprimirReporte"><i class="bi bi-clipboard-data"></i></button>

                            <button class="btn btn-success ms-1" style="font-size: 13px; padding: 7px" data-bs-toggle="modal" data-bs-target="#formModal" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-fecha="{{ $fecha }}" data-moneda="dolares" title="Editar pago" id="editarInput"><i class="bi bi-pencil"></i></button>

                            <button class="btn btn-primary abrirWhats whats_tabla ms-1" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-psnumero="{{ $ps->celular }}" data-fecha="{{ \Carbon\Carbon::parse($fecha)->formatLocalized('%B') }}" data-moneda="dolares" title="Mandar whats para pesos"><i class="bi bi-whatsapp"></i></button>

                            <button class="btn btn-primary abrirTrans whatsTrans_tabla ms-1" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-psnumero="{{ $ps->celular }}" data-fecha="{{ \Carbon\Carbon::parse($fecha)->formatLocalized('%B') }}" data-moneda="dolares" title="Mandar whats para dolares"><i class="bi bi-whatsapp"></i></button>

                            <input id="pagadoStatus" class="status form-check-input fs-6 m-0 p-0 ms-1" type="checkbox" data-id="{{ $pagos }}" @if ($status == "Pagado" || $status_convenio == "Pagado") {{"checked"}} @endif>
...
<div class="mt-4 contEuro" style="overflow-x: auto;">
    <table class="resumenPagoPs table table-striped table-bordered nowrap text-center" style="width: 100%; vertical-align: middle" id="resumenPagoPs">
        <thead style="vertical-align: middle">
            <tr>
                <th style="font-size: 14px !important; width: 30px !important;" data-priority="0" scope="col">PS</th>
                <th style="font-size: 14px !important; width: 30px !important;" data-priority="0" scope="col">Comisión (MXN)</th>
                <th style="font-size: 14px !important; width: 30px !important;" data-priority="0" scope="col">Comisión (EUR)</th>
                <th style="font-size: 14px !important; width: 30px !important;" data-priority="0" scope="col">Acciones</th>
            </tr>
        </thead>
        <tbody id="resumenBody" style="vertical-align: middle">
            @foreach ($lista_ps as $ps)
                @php
                    ...
                    $comision = number_format($comision_ps * $euro, 2);
                    $comision_dolares = number_format($comision_ps, 2);
                @endphp
                @if ($comision_ps > 0)
                    <tr>
                        ...
                        <td style="font-size: 14px">${{ $comision_dolares }}</td>
                        <td class="d-flex align-items-center">
                            <button title="Información de pago" class="btn btn-dark nota ms-1" style="font-size: 13px; padding: 7px" data-pagoid="{{$pagos}}" data-contratoid="{{$contratoid}}" data-pesos="{{ $comision }}" data-dolares="{{$comision_dolares}}" data-numeropago="{{$numero_pago}}" data-numeropagoconvenio="{{$numero_pago_convenio}}" data-pagoidconvenio="{{$pagos_convenio}}" data-moneda="euros"> <i class="bi bi-journal-text"></i></button>
                            
                            <button class="btn btn-warning ms-1" style="font-size: 13px; padding: 7px" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-fecha="{{ $fecha }}" data-moneda="euros" title="Imprimir pago" id="imprimirReporte"><i class="bi bi-clipboard-data"></i></button>

                            <button class="btn btn-success ms-1" style="font-size: 13px; padding: 7px" data-bs-toggle="modal" data-bs-target="#formModal" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-fecha="{{ $fecha }}" data-moneda="euros" title="Editar pago" id="editarInput"><i class="bi bi-pencil"></i></button>

                            <button class="btn btn-primary abrirWhats whats_tabla ms-1" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-psnumero="{{ $ps->celular }}" data-fecha="{{ \Carbon\Carbon::parse($fecha)->formatLocalized('%B') }}" data-moneda="euros" title="Mandar whats para pesos"><i class="bi bi-whatsapp"></i></button>

                            <button class="btn btn-primary abrirTrans whatsTrans_tabla ms-1" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-psnumero="{{ $ps->celular }}" data-fecha="{{ \Carbon\Carbon::parse($fecha)->formatLocalized('%B') }}" data-moneda="euros" title="Mandar whats para dolares"><i class="bi bi-whatsapp"></i></button>
...
<div class="mt-4 contFranco" style="overflow-x: auto;">
    <table class="resumenPagoPs table table-striped table-bordered nowrap text-center" style="width: 100%; vertical-align: middle" id="resumenPagoPs">
        <thead style="vertical-align: middle">
            <tr>
                <th style="font-size: 14px !important; width: 30px !important;" data-priority="0" scope="col">PS</th>
                <th style="font-size: 14px !important; width: 30px !important;" data-priority="0" scope="col">Comisión (MXN)</th>
                <th style="font-size: 14px !important; width: 30px !important;" data-priority="0" scope="col">Comisión (CHF)</th>
                <th style="font-size: 14px !important; width: 30px !important;" data-priority="0" scope="col">Acciones</th>
            </tr>
        </thead>
        <tbody id="resumenBody" style="vertical-align: middle">
            @foreach ($lista_ps as $ps)
                @php
                    ...
                    $comision = number_format($comision_ps * $franco, 2);
                    $comision_dolares = number_format($comision_ps, 2);
                @endphp
                @if ($comision_ps > 0)
                    <tr>
                        ...
                        <td style="font-size: 14px">${{ $comision_dolares }}</td>
                        <td class="d-flex align-items-center">
                            <button title="Información de pago" class="btn btn-dark nota ms-1" style="font-size: 13px; padding: 7px" data-pagoid="{{$pagos}}" data-contratoid="{{$contratoid}}" data-pesos="{{ $comision }}" data-dolares="{{$comision_dolares}}" data-numeropago="{{$numero_pago}}" data-numeropagoconvenio="{{$numero_pago_convenio}}" data-pagoidconvenio="{{$pagos_convenio}}" data-moneda="francos"> <i class="bi bi-journal-text"></i></button>

                            <button class="btn btn-warning ms-1" style="font-size: 13px; padding: 7px" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-fecha="{{ $fecha }}" data-moneda="francos" title="Imprimir pago" id="imprimirReporte"><i class="bi bi-clipboard-data"></i></button>

                            <button class="btn btn-success ms-1" style="font-size: 13px; padding: 7px" data-bs-toggle="modal" data-bs-target="#formModal" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-fecha="{{ $fecha }}" data-moneda="francos" title="Editar pago" id="editarInput"><i class="bi bi-pencil"></i></button>

                            <button class="btn btn-primary abrirWhats whats_tabla ms-1" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-psnumero="{{ $ps->celular }}" data-fecha="{{ \Carbon\Carbon::parse($fecha)->formatLocalized('%B') }}" data-moneda="francos" title="Mandar whats para pesos"><i class="bi bi-whatsapp"></i></button>

                            <button class="btn btn-primary abrirTrans whatsTrans_tabla ms-1" data-ps="{{ $ps->nombre }} {{ $ps->apellido_p }} {{ $ps->apellido_m }}" data-comision="{{$comision}}" data-comisiondolares="{{ $comision_dolares }}" data-psnumero="{{ $ps->celular }}" data-fecha="{{ \Carbon\Carbon::parse($fecha)->formatLocalized('%B') }}" data-moneda="francos" title="Mandar whats para dolares"><i class="bi bi-whatsapp"></i></button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="notaForm" method="post">
                    ...
                    <input type="hidden" name="id_convenio" id="idConvenioInputNota">
                    ...
                    <input type="hidden" name="numero_pago" id="numeroPagoInputNota">
                    <input type="hidden" name="numero_pago_convenio" id="numeroPagoConvenioInputNota">
                    <div class="row">
