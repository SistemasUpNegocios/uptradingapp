...
@if (auth()->user()->is_cliente || auth()->user()->is_mexico || auth()->user()->is_cliente_ps_gold ||
auth()->user()->is_cliente_ps_silver)
    @php
    $ps_id = session("psid");
    @endphp
    @if($ps_id != $psid)
    <a href="" data-id="{{ $id }}" data-nombrecliente="{{ $clientenombre }}" data-operador="{{ $operador }}"
        ...
        data-tipopago="{{ $tipo_pago }}" data-comprobantepago="{{ $comprobante_pago }}" data-psnombre="{{ $psnombre }}" type="button" title="Vista previa"
